...
| Function                        | Description                                                  |
| ------------------------------- | ------------------------------------------------------------ |
| `classifyPaywallHeaders`        | `x402` vs `mpp` vs `unknown` from response headers           |
| `headersIndicateMppPayment`     | True if `WWW-Authenticate` uses Payment scheme               |
| `tempoSessionChallengeToPolicy` | Extract amount/recipient from parsed tempo/session challenge |
| `tempoSessionProbeFromHeaders`  | Parse MPP session challenge from 402 headers (probes)        |
| `interceptMpp402Response`       | Policy check for MPP Tempo session 402                       |
| `fetchWithMppPolicyCheck`       | Initial fetch + MPP session policy pre-check                 |


Constants: `tempoChainId`, `tempoMainnet`, `tempoModerato`, `tempoEscrowContract`, `resolveTempoChain`, `resolveTempoChainId`, etc.
