...
- **Foundry** (forge, cast)
- **pnpm**
- `.env` with `PRIVATE_KEY` and RPC URL

Copy `.env.example` to `.env`, fill values, then `ln -sf ../.env contracts/.env` for forge. Never commit `.env`; use `chmod 600 .env`.
...
**Env:** `PRIVATE_KEY`, `BASE_SEPOLIA_RPC`. Optional: `AEP_TREASURY_ADDRESS` (defaults to address derived from PRIVATE_KEY).
...
4. Deploys first AEP account (owner derived from PRIVATE_KEY)
...
**Env:** `PRIVATE_KEY`, `BASE_MAINNET_RPC` (e.g. `https://mainnet.base.org`). Optional: `AEP_TREASURY_ADDRESS`. **Use multisig for treasury and owner in production.**
...
- [ ] [Threat Model](THREAT-MODEL.md) reviewed
