...
import { getSignerAccount } from "@aep/keystore";
import {
  createAccount,
  getAccountAddress,
  checkPolicy,
...
import { loadProviders, probeX402Endpoint } from "@aep/indexer";
import { runMonitor } from "@aep/monitor";
...
program
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      config.identityRegistryAddress = ERC8004_BASE_SEPOLIA.identityRegistry;
      config.reputationRegistryAddress = ERC8004_BASE_SEPOLIA.reputationRegistry;
...
counterparty
  ...
  .option("--registry <address>", "ReputationRegistry address", ERC8004_BASE_SEPOLIA.reputationRegistry)
  .option("-r, --rpc <url>", "RPC URL", DEFAULT_RPC)
  .action(async (opts) => {
    requireAddress(opts.registry, "registry");
    ...
    try {
      const summary = await getReputationSummary(agentId, {
        reputationRegistryAddress: opts.registry as `0x${string}`,
        rpcUrl: opts.rpc ?? config.rpcUrl ?? DEFAULT_RPC,
        chain: baseSepolia,
      });
...
credit
  ...
  .option("--token <address>", "Token address", USDC_BASE_SEPOLIA)
  ...
  .action(async (opts) => {
    ...
    requireAddress(opts.token, "token");
  ...
  .action(async (opts) => {
    ...
    try {
      const { escrow: escrowAddr, txHash } = await createEscrow({
        ...
        token: opts.token as `0x${string}`,
        validationRegistry: ERC8004_BASE_SEPOLIA.validationRegistry,
...
sla
  ...
  .option("--token <address>", "Stake token address", USDC_BASE_SEPOLIA)
  ...
  .action(async (opts) => {
    ...
    try {
      const { sla: slaAddr, txHash } = await createSLA({
        ...
        consumer: opts.consumer as `0x${string}`,
        providerAgentId: BigInt(opts.providerAgentId),
        stakeToken: opts.token as `0x${string}`,
  ...
  .action(async (opts) => {
    ...
    try {
      const hash = await slaDeclareBreach(
        ...
        opts.requestHash as `0x${string}`,
        { rpcUrl: opts.rpc ?? config.rpcUrl ?? DEFAULT_RPC, privateKey }
      );
      console.log("Tx:", hash);
    } catch (err) {
