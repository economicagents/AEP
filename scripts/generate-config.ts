#!/usr/bin/env npx tsx
/**
 * AEP config.json generator for Base Sepolia and Base Mainnet.
 * Reads addresses from env vars or JSON file, outputs complete config to stdout.
 *
 * Env vars:
 *   CHAIN_ID (84532 Sepolia, 8453 Mainnet), AEP_ACCOUNT_FACTORY, AEP_ACCOUNT_IMPL (implementation),
 *   CREDIT_FACILITY_FACTORY, ESCROW_FACTORY, REVENUE_SPLITTER_FACTORY, SLA_FACTORY, RPC_URL, TREASURY,
 *   OWNER (or derived from AEP_KEYSTORE_ACCOUNT or PRIVATE_KEY)
 *
 * Or: --from-json <path> to read addresses from a JSON file.
 *
 * Usage:
 *   CHAIN_ID=8453 AEP_ACCOUNT_FACTORY=0x... ... node generate-config.ts
 *   node generate-config.ts --from-json deployment.json
 */
...
function loadAddresses(): Addresses {
  ...
  return {
    ...
    owner: (() => {
      ...
      if (process.env.owner) return process.env.owner;
      const keystoreAccount = process.env.AEP_KEYSTORE_ACCOUNT ?? process.env.FOUNDRY_ACCOUNT;
      if (keystoreAccount) {
        try {
          return execFileSync("cast", ["wallet", "address", "--account", keystoreAccount], {
            encoding: "utf-8",
          }).trim();
        } catch {
...
function buildConfig(addr: Addresses): object {
  ...
  const chainId = parseInt(process.env.CHAIN_ID ?? String(addr.chainId ?? 84532), 10);
  const chain =
  ...
  const impl = process.env.AEP_ACCOUNT_IMPL?.trim();
  if (impl) {
    config.aepAccountImplementationAddress = impl;
    config.implementationAddress = impl;
  }
