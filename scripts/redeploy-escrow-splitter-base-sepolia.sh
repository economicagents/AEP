...
# Updates ~/.aep/config.json, docs/guides/deployment.md (Sepolia table), and smoke test addresses.
...
# Update docs/guides/deployment.md (Sepolia table) and packages/smoke-tests/src/addresses.ts
...
const deployDoc = process.env.REPO_ROOT + '/docs/guides/deployment.md';
if (fs.existsSync(deployDoc)) {
  ...
  console.log('Updated docs/guides/deployment.md');
}
