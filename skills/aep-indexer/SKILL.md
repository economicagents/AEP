...
compatibility: Requires Node.js. Run from packages/indexer or install @aep/indexer. Optional better-sqlite3, sqlite-vec for vector search.
metadata:
  version: 1.0.0
  openclaw:
    emoji: "📇"
...
From monorepo: run `node dist/cli.js` from packages/indexer. With `pnpm add -g @aep/indexer`, the `aep-index` bin is available. The `aep provider probe` command comes from aep-cli (separate package).
